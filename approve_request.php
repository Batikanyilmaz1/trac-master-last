<?php
	...
	$explanation = $_GET['txt_explanation'] ?? 'NULL';

	$status_id = 12;

	if($request_uuid && $request_approver_detail_uuid) {
			...
			if($userid) {
				...
				if($authorized_person_id) {
					...
					if($result) {
						...
						$continue = $row['iscontinue'];

						if($updatedrowcount && $continue) {
							$new_uuid = gen_uuid();
							$_SESSION['request']['approver']['uuid'] = $new_uuid;
							$_SESSION['request']['uuid'] = $request_uuid;
