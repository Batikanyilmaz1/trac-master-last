...
<head>
    ...
    <script type="text/javascript">
        ...
        function toggle(direction) {
            if(direction == 'expand') {
                $('div[id*="div_reservation_"]').slideDown('slow');
            } else if(direction == 'collapse') {
                $('div[id*="div_reservation_"]').slideUp('slow');
            }
            gototop();
        }
        ...
        function set_status(status_id = $('#sel_status').val(), start_date = $('#inp_start_date').val(), end_date = $('#inp_end_date').val()) {
            if(status_id != '') {
                $('#div_reservations').load('./get_reservation_list.php?list_type=' + <?php echo $list_type; ?> + '&status_id=' + status_id + '&start_date=' + start_date + '&end_date=' + end_date);
            }
        }

        function slide_reservation(index) {
            $('#div_reservation_top_' + index).slideToggle('slow');
            $('#div_reservation_' + index).slideToggle('slow');
            $('html, body').animate({ scrollTop: $('#div_reservation_' + index).parent().offset().top }, 'slow');
        }
        ...
        function toggle_request_detail(item) {
            ...
            if(item.attr('value').indexOf('Göster') > -1) {
                ...
                if($('#div_request_detail_' + index).html() == '') {
                    $('#div_request_detail_' + index).load('./get_request.php?request_id=' + item.attr('requestid'),
                        function(response, status, xhr) {
                            if (status == "success") {
                                $('#div_request_detail_' + index).slideToggle('slow');
                            } else {
        ...
        function open_manager_process(item) {
            var item1 = $('#div_approval_buttons_' + item.attr('index'));
            var item2 = $('#div_manager_process_' + item.attr('index'));
            var reservationid = item.attr('reservationid');

            item2.load('./decide_reservation.php?index=' + item.attr('index') + '&reservationid=' + reservationid);
            item1.slideUp('slow');
        ...
        function cancel_reservation(item) {
            if(check_form(item)) {
                ...
                if(confirm('"' + reservationid + '" numaralı rezervasyon iptal edilecek.\nDevam etmek istiyor musunuz?')) {
                    var index = item.attr('index');
                    var form = item.closest('form');
                    var formData = form.serialize();

                    $.getJSON('./cancel_reservation.php?' + formData,
                        function(data) {
                            if(data.Rows) {
                                ...
                                if(rowData.status == '1') {
                                    ...
                                    setTimeout(function() {
                                        $('#div_reservation_top_' + index).slideUp('slow');
                                        $('#div_reservation_' + index).slideUp('slow',
                                            function() {
                                                alert('"' + reservationid + '" numaralı rezervasyon iptal edildi.');
...
<body>
    ...
    <div id="div_main_area" hidden>
        <div id="div_main_frame" class="main_frame">
            ...
            <div class="heading">ULAŞIM ve KONAKLAMA REZERVASYON LİSTESİ</div>
                    ...
                    <div style="display: flex; width: 90%; height: 38px;">
                        ...
                        <div style="width: 65%;">
                            <select id="sel_status" name="sel_status" label="div_lbl0" area="area1" action="go" class="inp" onChange="set_status();">
                                ...
                                <option value="21" <?php if($status == '21') { echo 'selected'; } ?>>Aktif</option>
                                <option value="22" <?php if($status == '22') { echo 'selected'; } ?>>İptal Edildi</option>
                            </select>
                        </div>
                    </div>
                    <div style="display: flex; width: 90%; height: 38px;">
